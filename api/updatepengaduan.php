<?php
    ...
    if($cek == 1){
        if($nsung == null || $nbend == null|| $peng==null){
            echo json_encode(array("response"=>"Mohon isi data dengan benar"));
        }else{
            $response = array(
                "response"=>"Gagal",
                "status"=>false
            ...
            $queri=mysqli_query($conn,"UPDATE `db_pengaduan` SET `id_user`='$idus', `id_daerah`='$idaer', `nama_sungaiPeng`='$nsung', `nama_bendungPeng`='$nbend', `pengaduan`='$peng', `foto_pengaduan`='$gambarpeng', 
             `respon_pengaduan`='', `date_pengaduan`=curdate(), `waktu_pengaduan`=CURRENT_TIME(), `lokasi_pengaduan`='$lokasi', `status_pengaduan`='Belum Direspon' WHERE id_pengaduan=$idpengadu");
